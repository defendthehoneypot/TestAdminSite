...
pfSense (2.4+) provides native URL Table Aliases that can fetch IP addresses from HTTP/HTTPS URLs. Additionally, the pfBlockerNG package offers enhanced DIPL-like functionality. This guide covers both approaches.
...
- **URL**: Your DIPL URL with embedded credentials
...
- **URL**: DIPL URL with embedded credentials (same format as Method 1)
...
3. Configure:  
   ...
   Description: Update Vesper DIPL alias
   ```
4. Click **Save**  
...
3. Both nodes will independently fetch the DIPL  
...
2. Use the same URL (if DIPL returns IPv6 addresses)  
3. Or create separate IPv6 DIPL if available  
