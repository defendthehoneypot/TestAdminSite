...
Configure Palo Alto Networks (PAN-OS) firewalls to use Vesper Secure's External Dynamic List for automated ACL updates.
...
- **Source URL**: Your unique EDL URL from Vesper admin panel
- **Check for updates**: 
...
!!! tip "Empty List is OK"
    If no users have registered yet, the EDL may be empty. A successful connection to an empty list is still a success.
...
- **Source Address**: Select the EDL object you created
...
!!! success "Recommended Settings"
    - **EDL Refresh**: 15 minutes for balanced approach
...
!!! info "Monitoring"
    - Set up alerts for EDL update failures
...
!!! tip "Optimization"
    ...
    - Schedule regular EDL connectivity tests
